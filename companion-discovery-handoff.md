...
**Milestone 2 — DONE (revised to a chained turnover).** The companion runs as part of one daily turnover, not on its own timer: `daily-poem.timer` fires at 01:00 → `daily-poem.service` renders the poem → it `Wants=daily-companion.service`, which runs `After=` it and writes `out/companion.{png,json}`. So the poem is always selected/rendered first and the companion is paired to that exact poem (fixes the earlier two-independent-picks fragility). `systemd/daily-companion.timer` was removed. Needs `ANTHROPIC_API_KEY` in `/home/pi/daily-poem.env`.
...
**Deployed on the Pi (`dailypoem.local`, user `pi`, repo `/home/pi/daily-poem`).** Units installed: `daily-poem.timer` (01:00, drives the whole turnover), `daily-poem.service` (renders poem, `Wants=` the companion), `daily-companion.service` (chained `After=`, no own timer), `daily-companion-server.service` (:8080, enabled on boot). `ANTHROPIC_API_KEY` added to `/home/pi/daily-poem.env` (mode 600). Verified end-to-end on hardware: the selection runs from the live Notion poem, and both `POST /companion` (text page 2) and `POST /poem` pushed to the panel successfully. Bug fixed during deploy: the rank model prefaces its JSON with prose, which crashed `json.loads`; parsing now extracts the `{...}` object (commit 16dac6f).
...
**Still to build / open:**
- **Render-gate calibration** (the deferred mud-detector rework) — best done against real dithered output on the panel, since the Mac preview only approximates Spectra-6. **First hardware data point (2026-06-29):** an image companion (Met "Magical Stela with Horus the Child") looked *good* on the panel — crisp, not muddy — even though the Mac preview of such images shows heavy red/green dither noise. So the previews are pessimistic and the gate is **not** obviously broken in practice. Recommended approach now: *observe the daily output over a week or two* rather than a dedicated rework; the failure mode to watch for is dark oil paintings / photographs (the David "Death of Socrates" preview was true mud). Only tighten the detector if a genuinely muddy image reaches the panel.
